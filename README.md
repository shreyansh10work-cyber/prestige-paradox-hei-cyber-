...
Programme: MSc Business Analytics · Institution: University of Greenwich
Module: BUSI 1783 — Business Analytics Project · Supervisor: Dr Raunak Mishra
...
Repository contents
File	Description
Codebook.ipynb	Main analytical notebook: GDELT extraction, cleaning, OpenAlex merge, regression, and figures.
Codebook 2.0.ipynb	Revised / extended notebook.
Big Query SQL script	BigQuery extraction query for the GDELT GKG pull.
...
README.md	This file.
Final dataset variables (prestige_paradox_dataset_741.csv)
Column	Description
...
Method in brief
GDELT BigQuery extraction — gdelt-bq.gdeltv2.gkg_partitioned, 1 Jan 2020 – 31 Dec 2024, filtered for HEI and cybersecurity keywords (552,860 rows).
Eight-stage pipeline — raw extraction → noise removal → HEI keyword filter → cyber URL filter → URL deduplication → 7-day event-window deduplication → 3+ minimum-frequency filter (11,903 records across 1,280 names) → OpenAlex fuzzy merge at threshold 90/100 (final 741 universities).
...
Modelling — Negative Binomial regression (overdispersion ratio = 22.81 rules out Poisson), with 20 country dummies and log-citation count as the primary predictor.
Robustness — five-optimiser stability check plus a collinearity-affected specification adding log_works_count.
Why 741 and not 742
...
Python 3. Required: pandas, numpy, statsmodels, thefuzz, requests, matplotlib, seaborn, scipy, google-cloud-bigquery.
...
Pseudo R² = 0.075 — prestige and country explain roughly 7.5% of deviance.
Citation
